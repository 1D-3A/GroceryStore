...
def InitialData(conn):
    ...
    if c.fetchone()[0] == 0:
        ...
        c.execute("INSERT INTO Customers (FirstName, LastName, Phone, PasswordHash, IsAdmin) VALUES ('John', 'Doe', '897-516-9503', %s, 1)", (adminPW,))
        
        custPW = hashlib.sha256("shopper1738".encode()).hexdigest()
        c.execute("INSERT INTO Customers (FirstName, LastName, Phone, PasswordHash, IsAdmin) VALUES ('Jane', 'Doe', '973-377-0514', %s, 0)", (custPW,))
...
def AuthUser(conn):
    ...
    try:
        c = conn.cursor()
        c.execute(" SELECT CustomerID, IsAdmin FROM Customers WHERE FirstName = %s AND LastName = %s AND PasswordHash = %s", (first, last, hashPW))
        user = c.fetchone()
...
def AddProduct(conn):
    try:
        ...
        code = '''INSERT INTO Products (SKU, ProductName, Price, StockQuantity, CategoryID, SupplierID) VALUES (%s, %s, %s, %s, %s, %s)'''
        c = conn.cursor()
...
def UpdateStock(conn):
    try:
        ...
        code = '''UPDATE Products SET StockQuantity = %s WHERE ProductID = %s'''
        c = conn.cursor()
...
def PlaceOrder(conn, user):
    try:
        ...
        while True:
            ...
            c = conn.cursor()
            c.execute("SELECT Price, ProductName, StockQuantity FROM Products WHERE ProductID = %s", (prodID,))
            product = c.fetchone()
        ...
        c = conn.cursor()
        c.execute("INSERT INTO Orders (CustomerID, OrderDate, TotalAmount) VALUES(%s,%s,%s)", (cust, orderDate, total))
        orderID = c.lastrowid
        
        for item in items:
            c.execute("INSERT INTO OrderItems (OrderID, ProductID, Quantity, ItemPrice) VALUES(%s,%s,%s,%s)",
                        (orderID, item[0], item[1], item[2]))
            
            c.execute("UPDATE Products SET StockQuantity = StockQuantity - %s WHERE ProductID = %s",
                        (item[1], item[0]))
...
def ViewOrders(conn, user):
    try:
        ...
        if user['isAdmin'] == 1:
            ...
            c.execute("SELECT OrderID, CustomerID, OrderDate, TotalAmount FROM Orders WHERE CustomerID = %s", (user['id'],))
            print("\n--- My Order History ---")
